...
module.exports = {
  ...
  theme: {
    screens: {
      // Breakpoint (from old config, unchanged)
      xs: "320px",
    ...
    extend: {
      // Colors (merge old and new)
      colors: {
        // Old colors (preserved)
        ...
        // New colors (from new config, merged)
        ...
        // Update darkblue to match new config if needed, or keep old value
        darkblue: "#1a2b4e", // From new config, overriding old "#202745" if desired
        orange: {
          DEFAULT: "#f26a3d", // From new config, overriding old "#FF5B00" if desired
          50: "#fff7ed",
          100: "#ffedd5",
          200: "#fed7aa",
          300: "#fdba74",
      ...
      // area (width, height, maxWidth, maxHeight - preserved from old config)
      width: {
        "1px": "1px",
        "14px": ".875rem",
        "18px": "1.125rem",
      ...
      // spacing (gap, padding, margin - preserved from old config)
      gap: {
        "5px": ".3125rem",
        "10px": "10px",
        "11px": "11px",
      ...
      // typography (fontSize, lineHeight, letterSpacing - preserved from old config)
      fontSize: {
        "size-10": ".625rem",
        "size-11": ".6875rem",
        "size-12": ".75rem",
      ...
      // background (preserved from old config)
      backgroundImage: {
        register: "url('../assets/images/register/register__bg.png')"
      },
      // shadow (preserved from old config)
      boxShadow: {
      ...
      // position (preserved from old config)
      zIndex: {
        "-1": "-1",
        1: "1",
        xxsmall: "1000",
      ...
      // transform (translate - preserved from old config)
      translate: {
        "5px": "5px",
        "50px": "3.125rem",
        "30px": "1.875rem"
      },
      // border (width, radius - preserved from old config)
      borderWidth: {
        "3px": "3px",
        "5px": "5px",
        9: "9px",
      ...
      borderRadius: {
        ...
        lg2: "10px",
        100: "100%",
        // New borderRadius from new config (merged)
        lg: "var(--radius)",
      ...
      // animation (merge old and new)
      keyframes: {
        // Old keyframes (preserved)
        "marquee-left": {
        ...
        // New keyframes (from new config)
        // Add drop-in animation for the card's initial appearance
        "drop-in": {
        ...
        "subtle-bounce": {
          ...
          "50%": { transform: "translateY(-5px)" } // Bounce up by 5px
        },
        "accordion-down": {
          from: { height: 0 },
      ...
      animation: {
        // Old animations (preserved)
        "marquee-left": "marquee-left 950s linear infinite", // alumini
        "marquee-right": "marquee-right 950s linear infinite", // alumini
        ...
        // New animations (from new config)
  ...
  // components (preserved from old config)
  corePlugins: {
    container: false
  },
  // plugins (merge old and new)
  plugins: [
    // Old plugins (containers and shadows)
